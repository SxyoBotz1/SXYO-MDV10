    ...
    try {
      ...
      try {
        ...
        if (chat) {
          ...
          if (!('antiBadword' in chat)) chat.antiBadword = falsr
...
`.trim(), wm, 'Store', '.store', 'Menu', '.menu', m.message, {
  ...
  async GroupUpdate({ jid, desc, descId, descTime, descOwner, announce }) {
    ...
    ketik *.off desc* untuk mematikan pesan ini
        `.trim()
    this.send2Button(jid, caption, wm, 'Store', '.store', 'Menu', '.menu', { contextInfo: { mentionedJid: this.parseMention(caption) } })
...
╰─────────────────֍`,
  ...
  if (msg) return conn.send3Button(m.chat, msg, '📮 Silahkan pilih menu lain dibawah ini', 'Group Bot', '.allgc', 'Store Bot', '.store', 'Rules Bot', '.rules', m)
 let unreg = {
  unreg: `╭─֍〔 𝐔𝐍𝐑𝐄𝐆𝐈𝐒𝐓𝐄𝐑 〕֍─
...
╰────────֍ `
 ...
 if (unreg) return conn.sendButton(m.chat, unreg, '📮 Silahkan Klik reg dibawah ini', 'Register', '.daftar', m)
  }
